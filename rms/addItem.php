...
if (isset($_POST['addItem'])) {
    ...
    $sql = "INSERT INTO item (`item_id`, `item_name`, `unit`, `price`) VALUES ('$itemCode', '$itemName', '$unit', '$sellingPrice')";
    ...
    <div class="container container-custom mt-3 ">
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <div class="form-outline form-white mb-3 fs-4">
                ...
                <input type="text" class="form-control border-2 rounded-3" name="itemCode" placeholder="Add item code"
                    style="border-color: #EC6509; height: 60px;" required>
            </div>
            <div class="form-outline form-white mb-3 fs-4">
                <label class="form-label" for="itemName">Item Name</label>
                <input type="text" class="form-control border-2 rounded-3" name="itemName" placeholder="Add item name"
                    style="border-color: #EC6509; height: 60px;" required>
            </div>
            <div class="form-outline form-white mb-3 fs-4">
                <label class="form-label" for="unit">Unit</label>
                <input type="text" class="form-control border-2 rounded-3" name="unit" placeholder="Add unit (gm)"
                    style="border-color: #EC6509; height: 60px;">
            </div>
            <div class="form-outline form-white mb-3 fs-4">
                <label class="form-label" for="sellingPrice">Selling price</label>
                <input type="text" class="form-control border-2 rounded-3" name="sellingPrice" placeholder="Add selling price"
                    style="border-color: #EC6509; height: 60px;" required>
            </div>
            <div class="text-center mt-5">
                <button name="addItem" type="submit" class="btn btn-warning fw-semibold border-dark fs-5"
